...
"""
users -> id_user, username, e_mail, phone, userpassword, retrieve_code, user_rank
...
def sign_up(name,email,phone,password,window,rank=1):
    conexao, cursor = connection()
    query = "SELECT * FROM users WHERE e_mail = %s OR username = %s"
...
def send_code(email,window):
    ...
    try:
        ...
        messagebox.showinfo("Sucesso", "Código enviado no e-mail e SMS! ")
    except Exception as e:
        messagebox.showinfo("Sucesso", "Código enviado apenas no e-mail! ")
    open_gui(window,12)
...
def update_information(window,e_mail, name, phone, password):
    ...
    if name.strip() and not("Digite" in name):
        ...
        if os.path.exists(old_folder_path) and not os.path.exists(new_folder_path):
            ...
            messagebox.showerror("Erro", "Foi utilizado o mesmo nome.")
            name = old_username
        ...
        query_path = ("""UPDATE documents SET doc_path = REPLACE(doc_path,%s, %s)
                        WHERE id_user = %s;""")
        
        cursor.execute(query_path, (old_username, name, id_user))
        conexao.commit()
